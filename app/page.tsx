...
const PHOTOS = [
  ...
  { src: '/photos/photo5.jpg', stamp: '12 - 25 - 23', cap: 'first christmas', loc: 'HOME', rot: -1.5 },
]
...
export default function HomePage() {
  ...
  const NotecardTape = () => <div style={{ position: 'absolute', width: 34, height: 10, background: 'rgba(255,235,170,0.8)', border: '0.5px solid rgba(200,165,80,0.3)', borderRadius: 1, top: -5, left: '50%', transform: 'translateX(-50%)' }} />
  ...
  const StoryNotecard = ({ mobile }: { mobile: boolean }) => (
    <div style={{ background: '#FDFAF5', border: '0.5px solid rgba(43,42,40,0.1)', padding: mobile ? '14px 16px' : '11px 13px', boxShadow: '0 2px 6px rgba(43,42,40,0.08)', position: 'relative', ...(mobile ? {} : { transform: 'rotate(1.5deg)', width: 175 }) }}>
      <NotecardTape />
      {STORY.map((p, i) => (
  ...
  const FeatureNotecard = ({ mobile }: { mobile: boolean }) => (
    <div style={{ background: '#FDFAF5', border: '0.5px solid rgba(43,42,40,0.1)', padding: mobile ? '14px 16px' : '11px 13px', boxShadow: '0 2px 6px rgba(43,42,40,0.08)', position: 'relative', ...(mobile ? {} : { transform: 'rotate(-1.5deg)', width: 182 }) }}>
  ...
  return (
    <div style={{ width: '100%', overflowX: 'hidden' }}>
      ...
      {/* How it works bar */}
      <div style={{ background: '#EFE8DF', borderBottom: '1px solid rgba(43,42,40,0.08)', padding: '10px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '0 12px', borderRight: i < 3 ? '1px solid rgba(43,42,40,0.1)' : 'none', flexShrink: 0 }}>
      ...
      {!isMobile && (
        <div style={{ position: 'relative', height: 440, background: '#EDE6DC', overflow: 'hidden', width: '100%' }}>
          {PHOTOS.map((p, i) => {
            const lefts = ['1%', '16%', '32%', '48%', '64%']
            const tops = [12, 6, 14, 8, 16]
            const ws = [150, 145, 150, 142, 148]
            const hs = [196, 188, 194, 184, 192]
            return (
              <div key={i} style={{ position: 'absolute', left: lefts[i], top: tops[i], zIndex: i + 1, background: 'white', padding: '6px 6px 22px', transform: `rotate(${p.rot}deg)`, boxShadow: '0 2px 8px rgba(43,42,40,0.1)' }}>
                ...
                <img src={p.src} alt={p.cap ?? 'memory'} style={{ width: ws[i], height: hs[i], objectFit: 'cover', display: 'block' }} loading="eager" />
                <div style={{ position: 'absolute', bottom: p.loc ? 25 : 24, right: p.loc ? undefined : 7, left: p.loc ? 6 : undefined, fontFamily: 'Courier New, monospace', color: '#E8841A', fontWeight: 700, fontSize: 8, lineHeight: 1.4, letterSpacing: '0.06em', textShadow: '0 0 3px rgba(232,132,26,0.4)' }}>
          ...
          {/* Story notecard - overlaps between photo 1 and 2 */}
          ...
          <div style={{ position: 'absolute', right: 208, bottom: 18, fontFamily: 'Courier New, monospace', fontSize: 9, color: '#8A6F5A', fontStyle: 'italic', transform: 'rotate(2deg)', zIndex: 10 }}>2022 - 2024</div>
        </div>
      )}
          ...
          <div style={{ display: 'flex', gap: 12, overflowX: 'auto', padding: '16px 16px 8px', scrollbarWidth: 'none' }}>
            {PHOTOS.map((p, i) => (
              <div key={i} style={{ background: 'white', padding: '5px 5px 20px', transform: `rotate(${p.rot}deg)`, boxShadow: '0 2px 8px rgba(43,42,40,0.1)', position: 'relative', flexShrink: 0, width: 145 }}>
                ...
                <div style={{ position: 'absolute', bottom: p.loc ? 23 : 22, right: p.loc ? undefined : 6, left: p.loc ? 5 : undefined, fontFamily: 'Courier New, monospace', color: '#E8841A', fontWeight: 700, fontSize: 7.5, lineHeight: 1.4, letterSpacing: '0.06em', textShadow: '0 0 3px rgba(232,132,26,0.4)' }}>
                  {p.stamp}{p.loc && <><br />{p.loc}</>}
                </div>
