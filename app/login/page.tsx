...
export default function LoginPage() {
  ...
  // This used to ignore the result entirely, so when Supabase rejected the
  // request (for example while the Google provider was still switched off) the
  // button just did nothing and the only sign of trouble was a 400 in the
  // browser console. Surface the failure and point people at the magic link,
  // which always works.
  const handleGoogle = async () => {
    ...
    try {
      ...
      // A success hands back a URL and the browser leaves this page. If it
      // somehow doesn't, don't leave the button spinning forever.
      if (!data?.url) {
        setGoogleError("Google sign-in didn't start. Use your email below instead.")
        setGoogleLoading(false)
      }
    } catch (err: any) {
      setGoogleError(`Google sign-in failed: ${err?.message ?? 'unknown error'}`)
      setGoogleLoading(false)
    }
  }
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '56px 20px 80px', textAlign: 'center' }}>
      ...
      <button onClick={handleGoogle} disabled={googleLoading} style={{ width: '100%', padding: '14px 20px', background: 'white', border: '1px solid rgba(43,42,40,0.15)', borderRadius: 12, fontSize: 15, color: '#2B2A28', fontFamily: 'inherit', cursor: googleLoading ? 'default' : 'pointer', opacity: googleLoading ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 12 }}>
      ...
      {googleError && (
        <p style={{ fontSize: 12, color: '#C0392B', margin: '0 0 12px', lineHeight: 1.5 }}>{googleError}</p>
      )}
